...
import com.dtt.arenaofvalue.model.SkinHeroes;
...
public class DetailActivity extends AppCompatActivity {
    ...
    private String link, nameHero, url,idHero;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        idHero = intent.getStringExtra(Constance.ID_HEROES);
        Log.d("clickdetail", "onCreate: "+idHero);
        initviews();
...
//        linearLayout = findViewById(R.id.ll_detail);
    ...
    private ArrayList<Gem> parseGem(String link) {
        ...
        try {
            Gem gem = new Gem();
            ArrayList<SkinHeroes> arrSkin = new ArrayList<>();
            Document doc = Jsoup.connect(link).timeout(10000).get();
            /*
            ...
            for (int i = 0; i < skinHeroes.size() - 1; i++) {
                ...
                Log.d("tungasdas", "parseGem: " + "https://lienquan.garena.vn/" + id);
                skin.add(id);
            }
            for (int i = 1; i < skin.size(); i++) {
                String test = skin.get(i);
                String srcSkin = docSkinHero.getElementById(test).getElementsByTag("img").attr("src");
                SkinHeroes skinHero = new SkinHeroes("https://lienquan.garena.vn/" + srcSkin);
                arrSkin.add(skinHero);
                Log.d("tungasdas", "parseGem: " + "https://lienquan.garena.vn/" + srcSkin);
            }
            /*
                read Gem
             */
